...
public class Solution {
    public static void main(String[] args) {
        ...
        try {
            ...
            outputStream.flush();
            Human petrov = new Human("Petrov", new Asset("cat"), new Asset("AK"), new Asset("vodka"));
            ...
            System.out.println(petrov.equals(somePerson2));
            inputStream.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static class Human {
        ...
        public boolean equals(Human h)
        {
            ...
            {
                if (this.assets.size() == this.assets.size())
                {
                    for (int i = 0; i < assets.size(); i++)
                    {
                        if (this.assets.get(i).getName().equals(h.assets.get(i).getName()))
                        {
                            if (this.assets.get(i).getPrice() == h.assets.get(i).getPrice())
                            {
                                result = true;
                            } else result = false;
                        }else
